...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	float fAngle = (float)((2*M_PI)/a_nSubdivisions);

	for (int i = 0; i < a_nSubdivisions; i++) 
	{
		glm::vec3 start = glm::vec3(0, 0, 0);
		glm::vec3 first = glm::vec3(cos(i * fAngle) * a_fRadius, sin(i * fAngle) * a_fRadius, 0);
		glm::vec3 second = glm::vec3(cos((i + 1) * fAngle) * a_fRadius, sin((i + 1) * fAngle) * a_fRadius, 0);
		AddTri(start, first, second);
	}
